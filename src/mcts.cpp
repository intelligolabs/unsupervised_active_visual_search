...
bool MCTS::Update(int action, int observation, double reward) {
    ...
    if (Params.UseTransforms) AddTransforms(Root, beliefs);

    // If we still have no particles, fail
    if (beliefs.Empty() && (!vnode || vnode->Beliefs().Empty())) return false;
...
void MCTS::RolloutSearch() {
    ...
    Simulator.GenerateLegal(*BeliefState().GetSample(0), GetHistory(), legal,
                            GetStatus());
    ...
    for (int i = 0; i < Params.NumSimulations; i++) {
        ...
        bool terminal = Simulator.Step(*state, action, observation,
                                       immediateReward); 
...
double MCTS::SimulateQ(STATE& state, QNODE& qnode, int action) {
    ...
    if (Simulator.HasAlpha()) Simulator.UpdateAlpha(qnode, state);
    ...
    double totalReward =
        immediateReward + Simulator.GetDiscount() * delayedReward;
...
void MCTS::AddTransforms(VNODE* root, BELIEF_STATE& beliefs) {
    ...
    cout << "Params.NumTransforms:" << Params.NumTransforms << endl;
...
STATE* MCTS::CreateTransform() const {
    ...
    STATE* state = Root->Beliefs().CreateSample(Simulator);
    Simulator.LocalMove(*state, History, stepObs, Status);
    Simulator.Step(*state, History.Back().Action, stepObs, stepReward);
    if (stepObs != 1) return state;
...
inline double MCTS::FastUCB(int N, int n, double logN) const {
    if (InitialisedFastUCB && N < UCB_N && n < UCB_n) return UCB[N][n];
...
void MCTS::DisplayStatistics(ostream& ostr) const {
    ...
    if (Params.Verbose >= 2) {
        ...
        ostr << "Values after " << Params.NumSimulations << " simulations"
             << endl;
        DisplayValue(6, ostr);
    }
}
