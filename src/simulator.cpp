...
SIMULATOR::SIMULATOR()
    : Discount(1.0), NumActions(0), NumObservations(0), RewardRange(1.0) {}
...
bool SIMULATOR::LocalMove(STATE& state, const HISTORY& history, int stepObs,
...
int SIMULATOR::SelectRandom(const STATE& state, const HISTORY& history,
    ...
    if (Knowledge.RolloutLevel >= KNOWLEDGE::SMART) {
        ...
        if (!actions.empty()) return actions[Random(actions.size())];
    }
...
bool SIMULATOR::HasAlpha() const { return false; }
...
void SIMULATOR::DisplayBeliefs_t(const BELIEF_STATE& beliefState,
                                 ostream& ostr) const {}

void SIMULATOR::DisplayBeliefs_end(const BELIEF_STATE& beliefState,
                                   STATE& state, ostream& ostr) const {}

void SIMULATOR::DisplayBeliefs_t_end(const BELIEF_STATE& beliefState,
                                     STATE& state, ostream& ostr) const {}

void SIMULATOR::DisplayProbability() const {};
void SIMULATOR::SaveGaussVector(std::ostream& ostr) const {}
...
void SIMULATOR::DisplayObservation(const STATE& state, int observation,
                                   ostream& ostr) const {
